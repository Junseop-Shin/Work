...
## 전체 기술 선택 지도 — Native / 웹앱 / WASM 포함

RN·Flutter는 크로스플랫폼 두 갈래일 뿐, 실제 선택지는 더 넓다. 5종 종합:

| | 무엇 | 언어 | 코드 공유 | 성능 | 네이티브 느낌 |
|---|------|------|----------|------|-------------|
| **React Native** | JS로 네이티브 UI 조작 | JS/TS | iOS+Android(+웹) | 상 | 상 |
| **Flutter** | 자체 엔진으로 픽셀 렌더 | Dart | iOS+Android+웹+데스크톱 | 최상 | 중상 |
| **Native (Swift/Kotlin)** | OS 네이티브 | Swift / Kotlin | ❌ 각각 | 최상 | 최상 |
| **웹앱 (PWA/반응형)** | 브라우저 | JS/TS | 전부 (URL 하나) | 중 | 하 |
| **WASM** | 브라우저 내 고성능 연산 | Rust/C++/C# | 웹(+코드 재사용) | 최상(연산) | — |

### 각각의 본질

- **React Native (+Expo)** — "JS 팀의 모바일 진출". ✅ 웹 개발자 즉시 투입(React 그대로), npm 생태계, OTA, 코드 공유. ❌ JS↔네이티브 브릿지 오버헤드(무거운 그래픽 불리), 버전 업글 고통. 👉 React 팀 + 콘텐츠/폼 위주. Instagram, Discord
- **Flutter** — "픽셀을 직접 그린다". ✅ 자체 엔진으로 성능·플랫폼 간 픽셀 일관성 최상. ❌ Dart(생태계 작음), 앱 용량 큼, OS 관습과 미묘한 차이. 👉 브랜드 UI 일관성 + 고성능 애니메이션. Google Pay, 알리바바
- **Native (Swift/Kotlin)** — "최고를 양보 안 함". ✅ 성능·최신 OS 기능·관습 100%. ❌ iOS/Android 따로 = 인력 2배, 코드 공유 0. 👉 성능/하드웨어가 생명(카메라/AR/금융 보안) 또는 단일 플랫폼.
- **웹앱 (PWA/반응형/Next)** — "설치 없는 도달". ✅ 설치 마찰 0, 심사 없음(즉시 배포), SEO. ❌ 네이티브 기능 제약(BLE/백그라운드/푸시 약함), iOS PWA 빈약. 👉 콘텐츠/도구형 + 빠른 반복 + 도달 우선. (네이티브 기능 일부 필요 시 → WebView 래퍼로 보강)
- **WASM** — 앱 플랫폼이 아니라 "웹의 성능 보강기". UI 프레임워크가 아니라 브라우저에서 네이티브급 연산을 돌리는 실행 기술. ✅ C++/Rust 연산을 웹에서(이미지/영상 편집, 3D, 암호화), 기존 코드 재사용. ❌ DOM 직접 조작 불가(JS 경유), 번들 큼, UI 프레임워크로는 미성숙. 👉 웹에서 무거운 연산. Figma(C++→WASM), Photoshop Web

### 빠른 의사결정 트리

```
단일 플랫폼 + 성능/하드웨어가 생명   → Native (Swift/Kotlin)
React 팀 + 콘텐츠/폼 위주 앱        → React Native + Expo
UI 픽셀 일관성 + 고성능 애니메이션   → Flutter
설치 마찰 0 + 빠른 반복 + 도달 우선  → 웹앱 (PWA/Next)
기존 웹앱 + 네이티브 기능 일부 필요   → 웹앱 + WebView 래퍼
웹에서 무거운 연산(편집기/게임)      → 웹 + WASM
```

### 핵심 판단 축 3개

1. **팀 역량** — React 팀이면 RN, Dart 가능하면 Flutter, 둘 다 없고 성능 필요하면 Native. 가장 현실적인 결정 요인.
2. **성능 요구** — 하드웨어/그래픽 무거우면 Native·Flutter, 콘텐츠 위주면 RN·웹 충분.
3. **도달 vs 깊이** — "빨리 많은 사람에게"는 웹, "기기를 깊게 쓰는 경험"은 네이티브. 웹앱→네이티브 전환 경계는 보통 **푸시/백그라운드/BLE 같은 깊은 기기 기능, 또는 60fps 인터랙션**이 필요해질 때.
