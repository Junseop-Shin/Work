...
## 2. 다섯 개는 도구가 아니라 데이터셋이다

본편 6절이 다섯 KG를 나열하는데, 이것들이 어느 층에 있는 물건인지가 슬라이드에 안 나온다. RDF나
Neo4j 같은 이름과 섞이기 쉬운 자리다. 층이 셋이다.

| 층 | 무엇인가 | 예 |
|---|---|---|
| 데이터 모델·표준 | 명세서. 소프트웨어가 아니다 | RDF · OWL · SPARQL / LPG · Cypher |
| 그래프 DB 제품 | 설치해서 돌리는 저장 엔진 | GraphDB · Virtuoso · Jena · Stardog / Neo4j · TigerGraph |
| 지식그래프 | 실제로 채워진 내용물 | DBpedia · Freebase · OpenCyc · Wikidata · YAGO |

관계형에 비유하면 RDF와 SPARQL이 관계형 모델과 SQL, GraphDB와 Neo4j가 PostgreSQL과 MySQL,
DBpedia와 Wikidata가 그 안에 적재된 백과사전 데이터에 해당한다. 다섯 개는 제품이 아니라
내용물이다.

Neo4j는 이 회차 슬라이드 5·6번 하단에 각주로 등장하는데, 다뤄지는 대상이 아니라 온톨로지와 KG를
비교한 그림의 출처다. 다섯 KG 중 Neo4j 계열은 없다. Neo4j는 LPG 쪽이고 다섯은 전부 RDF 쪽이다.

### 측정 렌즈가 결과를 만든다

여기서 1절과 이어지는 것이 나온다. 논문은 다섯을 하나의 형식으로 통일해서 잰다.

> We consider the KG RDF export here

RDF로 맞춰야 `owl:disjointWith`나 `rdfs:range` 개수를 셀 수 있기 때문이다. 그런데 다섯의 네이티브
형태가 다 RDF는 아니다. Wikidata는 statement에 qualifier와 reference가 붙는 자체 데이터 모델이고
RDF는 거기서 내보낸 결과다. 논문도 직접 적는다.

> Wikidata does not use any `rdfs:range` restrictions. Within the Wikidata data model, ... not part
> of the Wikidata data model.

그래서 본편 10절의 `m_conRelat` = 0은 Wikidata에 제약이 없다는 뜻이 아니라 **RDF export에 그것이
실리지 않는다**는 뜻이다. Wikidata는 property constraint를 별도 시스템으로 관리하는데 그건 RDF로
나오지 않는다.

1절에서 정리한 것과 같은 구조다. 강제하는 층이 OWL 밖에 있으면, OWL을 보는 자로 재면 0이 나온다.
품질이 낮아서가 아니라 자가 그것을 못 재는 것이다. 점수를 읽을 때 **무엇을 재지 못하는 자인지**를
같이 봐야 한다.

### 저장 엔진 비교는 어느 논문도 하지 않는다

두 논문이 각각 다른 층을 본다.

| 논문 | 비교 대상 | 층 |
|---|---|---|
| Färber et al. | DBpedia · Freebase · OpenCyc · Wikidata · YAGO | 내용물 |
| Hofer et al. | FlexiFusion · dstlr · XI · AutoKnow · HKGB · SLOGERT · SAGA | 구축 파이프라인 도구 |

Hofer 쪽 일곱은 저장 엔진이 아니라 KG를 만드는 툴셋이라 또 다른 층이다. GraphDB냐 Neo4j냐 하는
저장 엔진 선택은 이 덱 전체에서 다뤄지지 않는다.
[S11-1 6절](S11-1-온톨로지가-하지-않는-일.md)에서 짚은 RDF와 LPG 선택 기준이 안 나오는 것과 같은
공백이다.

## 3. 같은 lifecycle을 두 번 배웠다
