...
이 회차의 제목이 "온톨로지 임베딩·정렬 실습"인데 실습은 정렬만 한다. [S15](S15A-OWL2Vec-star-문장-기반-임베딩.md)에서
배운 OWL2Vec\* 과 EL Embeddings를 돌려보려면 **mOWL** 로 가야 하고, 이번 회차에 나오지 않는다.

| Ch.5가 다룬 것 | 실습에서 돌려봤나 | 어디서 |
|---|---|---|
| KG 임베딩 (TransE · RotatE · R-GCN) | 돌렸다 | S17 · PyKEEN |
| 온톨로지 정렬 (BERTMap) | 돌렸다 | S18 · DeepOnto |
| **온톨로지 임베딩 (OWL2Vec\* · EL Embeddings)** | **못 돌렸다** | 도구가 없다 (mOWL 미확인) |
