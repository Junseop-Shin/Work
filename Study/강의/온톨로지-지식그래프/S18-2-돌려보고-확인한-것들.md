...
| 회차 제목: "온톨로지 임베딩·정렬 실습" | **정렬만 한다.** 임베딩 모델은 하나도 안 돌리고, DeepOnto에 애초에 없다 (8절) |
...
**절반만 채운다.** `deeponto.onto` 가 OWLAPI와 HermiT/ELK를 파이썬에서 그대로 쓰게 해주고,
`deeponto.align` 에 BERTMap이 실물로 들어 있다
([S16](S16A-OntoEA-온톨로지를-넣은-엔티티-정렬.md)에서 논문으로 본 그 시스템이다).

**그런데 온톨로지 임베딩 모델은 여전히 없다.** 모델 목록을 확인했더니
`owl2vec` · `elembeddings` 어느 것도 들어 있지 않다. `align` 은 정렬(매핑 표)을,
`complete` 는 subsumption 예측을 하고, **클래스마다 벡터를 만들어 내보내는 API가 없다**
([S18-1 4절](S18-1-읽는-데-필요한-것들.md)).

이 회차의 제목이 "온톨로지 임베딩·정렬 실습"인데 실습은 정렬만 한다.
[S15](S15A-OWL2Vec-star-문장-기반-임베딩.md)에서 배운 OWL2Vec\* 과 EL Embeddings를 돌려보려면
**mOWL**(`mowl-borg`)로 가야 한다. 설치해 확인했더니 `ELEmbeddings` · `ELBE` · `BoxEL` ·
`BoxSquaredEL` · `RandomWalkPlusW2VModel` 등 모델 14개와 `OWL2VecStarProjector` ·
`DL2VecProjector` 등 투영기 7개, 평가 모듈 6개가 들어 있다. **PyKEEN에 대응하는 온톨로지
임베딩 비교 프레임워크가 이것이고, 강의에 이 회차가 없다.**

> 목록만 확인했고 돌려보지는 않았다. 평가 모듈이 `PPIEvaluator`(단백질 상호작용) ·
> `GDAEvaluator`(유전자-질병 연관)라 생의학 쪽으로 기울어 있다는 것도 함께 적어둔다.

| Ch.5가 다룬 것 | 실습에서 돌려봤나 | 어디서 |
|---|---|---|
| KG 임베딩 (TransE · RotatE · R-GCN) | 돌렸다 | S17 · PyKEEN |
| 온톨로지 정렬 (BERTMap) | 돌렸다 | S18 · DeepOnto |
| **온톨로지 임베딩 (OWL2Vec\* · EL Embeddings)** | **못 돌렸다** | 강의에 회차가 없다. 도구는 있다 — mOWL |

**PyKEEN처럼 여러 모델을 갈아끼워 비교하는 틀은 아니다**
([S18-1 2.1절](S18-1-읽는-데-필요한-것들.md)). 파이프라인 클래스가 둘뿐이고,
모델 레지스트리도 하이퍼파라미터 탐색도 없다. `logmap` 모듈은 정렬기가 아니라
`run_logmap_repair` 하나를 위한 수리 서브루틴이다. 만든 그룹(Oxford KRR)이 자기 시스템을
배포하는 곳으로 보는 것이 맞다.
...
A-Box 링크 예측은 PyKEEN이고, 둘이 겹치지 않는다.

**단, 표에 "역할이 대칭이 아니다"를 함께 적어야 한다.** PyKEEN은 모델을 골라 비교하는
프레임워크이고 DeepOnto는 특정 시스템을 돌리는 툴킷이다. 나중에 이 표를 다시 볼 때
"T-Box 모델들을 DeepOnto로 비교하면 되겠다"로 읽으면 틀린다. 온톨로지 임베딩 모델을
여럿 비교하려면 mOWL 쪽을 봐야 하는데 이번에 다루지 않았다.

정렬 결과를 쓸 때의 조건은 **BERTMapLt를 항상 같이 돌려 기준선으로 둔다** 다.
이번 회차가 그것 없이는 "효과 0"을 못 봤을 자리다.
