...
> **PyKEEN에 대응하는 온톨로지 쪽 물건은 mOWL(`mowl-borg`)이다.** 설치해 확인했더니
> 모델 14개(`ELEmbeddings` · `ELBE` · `BoxEL` · `BoxSquaredEL` · `RandomWalkPlusW2VModel` ·
> `GraphPlusPyKEENModel` 등)와 투영기 7개(`OWL2VecStarProjector` · `DL2VecProjector` 등),
> 평가 모듈 6개를 갖고 있다. 투영기 · 걷기 · 모델을 갈아끼우는 구조라 PyKEEN과 같은 결이다.
> [S15](S15A-OWL2Vec-star-문장-기반-임베딩.md)에서 본 두 모델이 여기 실물로 있다.
> **강의에 이 회차가 없고, 목록만 확인했지 돌려보지는 않았다.**
...
**서로 다른 두 온톨로지에서 같은 개념을 찾아 짝지어주는 일**이다. 이 실습에서 쓴 데이터로
보면 이렇다.

```mermaid
flowchart LR
  subgraph EN["conference-en.owl"]
    E1["c-2541048-2862308<br/>tutorial"]
    E2["c-9012...<br/>member of committee"]
    E3["c-3344...<br/>abstract"]
  end
  subgraph PT["conference-pt.owl"]
    P1["c-2245653-0676275<br/>Tutorial"]
    P2["c-3456...<br/>membro do comite"]
    P3["c-7788...<br/>resumo"]
  end
  E1 -.->|"동일 개념"| P1
  E2 -.->|"동일 개념"| P2
...
**왜 이런 문제가 생기나.** 온톨로지는 각자 따로 만들어진다. 세계 공용 개념 등록소가 없으니
같은 것이 다른 IRI(만든 사람의 네임스페이스라 필연적이다), 다른 이름(번역 · 동의어 · 약어 ·
사내 용어), 다른 계층 위치를 갖는다. 두 온톨로지를 합치거나 가로질러 질의하려면 이 대응표가
반드시 있어야 하고, 없으면 남남인 파일 두 개다.

**산출물은 매핑 표다.**

```
SrcEntity                 TgtEntity                 Score   Relation
conference_en#c-2541048   conference_pt#c-2245653    1.0        =
```

`=` 는 동등(equivalent), `⊑` 는 포함(subsumption)이다. 대개 `=` 를 찾는다.

**왜 어려운가.**

| 문제 | 이 실습의 데이터에서 |
|---|---|
| 이름이 다르다 | `abstract` ↔ `resumo`. 겹치는 글자가 없다 |
| 이름이 같아도 뜻이 다를 수 있다 | 동음이의어 |
| 입도가 다르다 | 한쪽의 한 클래스가 저쪽의 둘에 걸친다 |
| 대응이 아예 없는 것도 있다 | 61개 클래스 중 정답이 있는 것은 59개 |
| 후보가 n×m 개다 | 61 × 61 = 3,721쌍. 실무 온톨로지는 수만 × 수만 |

마지막 줄이 BERTMap이 idf로 후보를 먼저 좁히는 이유다(4.3).

**개체 정렬(Entity Alignment)과 다르다.**
[S16A 1절](S16A-OntoEA-온톨로지를-넣은-엔티티-정렬.md)에서 이 둘을 갈랐다.

| | 온톨로지 정렬 | 개체 정렬 |
|---|---|---|
| 짝짓는 대상 | 개념 · 클래스 (T-Box) | 개체 · 인스턴스 (A-Box) |
| 예 | `Reviewer` 개념 = `Revisor` 개념 | 이 김철수 = 저 김철수 |
| 이 강의의 모델 | BERTMap · LogMap | OntoEA |

**방법의 계보.**

| | 무엇으로 판단하나 | 이 강의에서 |
|---|---|---|
| ① 문자열 유사도 | 이름이 얼마나 닮았나 | BERTMapLt |
| ② 구조 기반 | 계층에서 비슷한 자리에 있나 | — |
| ③ 논리 기반 | 추론으로 모순을 검사하며 반복 | LogMap (여기서는 repair로만) |
| ④ 언어모델 기반 | 이름의 뜻을 비교 | BERTMap · OntoEA |

**이 계보에서 S18의 결과가 읽힌다.** 데이터가 en↔pt 번역이라 ①이 원리적으로 통하지 않는데
(철자가 겹치는 정답쌍이 3.4%), ④를 붙여도 ①을 한 개도 넘지 못했다
([S18-2 6절](S18-2-돌려보고-확인한-것들.md)).

이 분야는 **OAEI**(Ontology Alignment Evaluation Initiative)가 매년 대회를 열고,
MultiFarm이 그 공식 벤치마크 중 하나다. `deeponto.align.oaei` 모듈이 그 채점 규격에
맞추는 배관이다.

> **실무에서 이 문제가 서는 자리** — 표준 온톨로지(IOF · BFO)에 사내 온톨로지를 붙일 때,
> 협력사 온톨로지와 합칠 때, 설비 벤더가 다른 두 라인의 스키마를 이을 때다.
> [`00-전체-파이프라인.md`](00-전체-파이프라인.md) 11절의 증분 개체 통합은 이것의
> A-Box 판(개체 정렬)이다.

### BERTMap은 임베딩 모델이 아니다
